...
class common_report_header(object):
    ...
    def _get_cost_center(self, data):
        names = []
        if data.get('form', False) and data['form'].get('cost_analytic_ids', False):
            self.cr.execute('select name from account_analytic_account   where id IN %s',(tuple(data['form']['cost_analytic_ids']),))
            names = [x for x, in self.cr.fetchall()]
        return names
            
    def _get_entry_type(self, data):
        names = []
        entry_dict = {
                      'car' : 'Car',
                      'branch' : 'Branch',
