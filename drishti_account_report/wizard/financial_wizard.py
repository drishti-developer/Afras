...
class accounting_report(osv.osv_memory):
    ...
    _columns = {
       ...
       # 'country_id': fields.many2one('res.country',  'Country'),
        ...
        'cost_analytic_ids' : fields.many2many('account.analytic.account','rel_report_analytic_id',
                                                'report_id', 'analytic_id',  'Cost Center', ) ,
        'fleet_report': fields.boolean('Include Cost Allocation',help="Include Cost Allocation base on car purchase value")        
#            'cost_analytic_ids' : fields.many2many('account.analytic.account', 'Cost Center', ) ,     
    ...
    def _print_report(self, cr, uid, ids, data, context=None):
        data['form'].update(self.read(cr, uid, ids, ['date_from_cmp',  'debit_credit', 'date_to_cmp',  'fiscalyear_id_cmp', 'period_from_cmp', 'period_to_cmp',  'filter_cmp', 'account_report_id', 'enable_filter', 'label_filter','level','cost_analytic_ids','child_cost_center','entry_type','fleet_report'], context=context)[0])
        if data['form']['fleet_report']:
            return {
            'type': 'ir.actions.report.xml',
            'report_name': 'account.fleet.report',
            'datas': data,
        }
        else:    
            return {
                'type': 'ir.actions.report.xml',
                'report_name': 'acc_financial_report',
                'datas': data,
            }
