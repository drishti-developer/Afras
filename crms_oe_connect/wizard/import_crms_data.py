...
class import_crms_data(osv.osv_memory):
    
    _name='import.crms.data'
    _columns={
    ...
    def import_data(self,cr,uid,ids,context=None):
        ...
        for line_list in line:
            line_value = line_list.split(',')
            if rownum == 0 :
                header = line_value
            else:
                value_dict = {}
                for row_value in line_value:
                    if row_value != '\n': 
                        value_dict[header[line_value.index(row_value)]] = row_value
        ...
        for i in range(1,sheet.nrows):
            ...
            if model_id:
                self.pool.get('fleet.vehicle').create(cr,uid,{'model_id':model_id,'license_plate':plate_number,'license_plate_arabic':plate_number,'barcode':serial,'vin_sn':vin_sn,
