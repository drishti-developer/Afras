...
class CountryState(osv.osv):
    ...
    _columns = {
                 ...
                 'code': fields.char('Region Code', size=3,
            help='The Region code in max. three chars.', required=True),
                'analytic_id' : fields.many2one('account.analytic.account','Analytic Account',),
...
class res_city_area(osv.osv): 
    ...
    _columns = {
                'name' : fields.char('Area Name',required=True),
                 'code': fields.char('Area Code', size=3,
            help='The Area code in max. three chars.', required=True),
...
class sale_shop(osv.osv):
    ...
    _columns = {
                ...
                #'location_id' : fields.related('warehouse_id','lot_stock_id',type='many2one', relation='stock.location',string='Branch Location',readonly=True),
    ...
    def create(self, cr, uid, values, context=None):
        id=super(sale_shop,self).create(cr, uid, values, context=context)
        ...
        self.pool.get('account.analytic.account').create(cr,uid,dic)
        return id
