...
class account_afras_cost_distribution(osv.osv):
    ...
    def create_plan_scheduler(self, cr, uid,vals, context=None):
        ...
        for parent in parent_analytic_objs:
            ...
            related_period = self.pool.get('account.period').search(cr,uid, [('company_id','=',parent.company_id.id),('date_start','=',from_date)]) 
...
class account_move_line(osv.osv):
    ...
    _columns = {
                'analytics_id':fields.many2one('account.afras.cost.distribution', 'Analytic Distribution'),
                'vehicle_id' : fields.many2one('fleet.vehicle','Vehicle'),
                'from_date' : fields.date('From Date'),
                'to_date' : fields.date('To Date'),
    ...
    def default_get(self, cr, uid, fields, context=None):
        ...
        analytic_acc_id = context.get('default_analytic_account_id',False)
        ctx = dict(context or {}, account_period_prefer_normal=True)
        period_ids = self.pool.get('account.period').find(cr, uid, context=ctx)
        dist_plan_id = 0
        if analytic_acc_id:
            acc_analytic_obj = self.pool.get('account.analytic.account').browse(cr,uid,analytic_acc_id)
            if acc_analytic_obj.use_distribution_plan:
                dist_plan_id = self.pool.get('account.afras.cost.distribution').search(cr,uid, ['&',('related_analytic','=',analytic_acc_id),('related_period','=',period_ids[0])], context=context)
                res.update({'analytics_id' : dist_plan_id}) 
    ...
    def create_analytic_lines(self, cr, uid, ids, context=None):
        ...
        for line in self.browse(cr, uid, ids, context=context):
           if line.analytics_id:
               ...
               for line2 in line.analytics_id.distribution_lines:
